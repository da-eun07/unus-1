...
#import module_that_outputs_signals #Fetch from cv. util func Connect to lidar as well
import cv2

# DRIVE
# MISSION_1 : AVOID OBSTACLE + TRAFFIC LIGHT
# MISSION_2 : PARKING
...
# ARDUINO CONNECTION
ser = ar_util.libARDUINO(port='COM8')
comm = ser.init('COM8', 9600)

# CAMERA CONNECTION
cam = cam_util.libCAMERA()
ch0, _ = cam.initial_setting_1080(cam0port=0, cam1port=1, capnum=1)
ch1, _ = cam.initial_setting_480(cam0port=1, cam1port=0, capnum=1)
...
while True:
    ...
    if (MODE == "DRIVE"):
        # print('Lets Drive')
        if steer == 'forward':
            send_command("0", speed=50)
        elif steer == 'right':
            send_command("1", speed=50)
        elif steer == 'left':
            send_command("-1", speed=50)
        else: # stop
            send_command("10", speed=50)

    elif (MODE == "MISSION_1"):
        # print('Lets Avoid Obstacle and Traffic Light')
        SUB_MODE = 0 # Avoiding

        if SUB_MODE == 0: # Avoiding
            # GET OBSTACLE INFO
            ...
            if OBSTACLE:
                ...
                if OBSTACLE == 'First Detect': # Left Obstacle
                    ...
                    send_command("-2", speed=500) ### FIX ME
            ...
            if TRAFFIC == 'RED' or 'YELLOW':  # Stop
                print("STOP")
            elif TRAFFIC == 'GREEN':  # Go
                print("GO")
